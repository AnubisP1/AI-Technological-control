...
_CATEGORY_PATTERNS: list[tuple[str, re.Pattern]] = [
    ...
    (
        # Клеймение и маркирование — ГОСТ 2.314-1968. Идёт до 'assembly':
        # формулировка «Клеймить К, маркировать Ч и порядковый номер сб.
        # ед.» относится к маркировке, хотя и упоминает сборочную единицу.
        "marking",
        re.compile(r"клеймит|клейм\w*|маркиров|товарн\w*\s+знак", re.IGNORECASE),
    ),
    (
        "assembly",
        re.compile(
            r"момент\w*\s+затяжк|фиксатор|зазор|сборк\w*\s+производ|по\s+стыковым",
            re.IGNORECASE,
        ),
    ),
    (
