"""Композитная реализация ITextGenerator, пробующая несколько
LLM-провайдеров по приоритету. В локальной (offline) сборке доступен
только локальный Qwen — цепочка сохранена как точка расширения.
