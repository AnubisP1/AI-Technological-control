...
Работает параллельно с облачным PolzaTextGenerator (Фаза 18, часть 2, см.
dev/QUESTIONS.md №15) — оба реализуют один порт с общим промптом
(review_summary_prompt.py), _build_text_generator() в routes.py выбирает
между ними по приоритету (Polza.ai первым, если настроен ключ, локальный
Qwen — фолбэком при отсутствии ключа/сети).
