...
-- eSUN PA-CF (полиамид, армированный углеволокном, для FDM) — добавлена
-- 2026-08-04 по запросу пользователя: группа NYLON_FDM (id=6) была
-- топ-рекомендацией автоподбора для класса PROPELLER, но не имела ни
-- одной реальной записи am_material, из-за чего экспертиза НСИ по
-- параметрам печати (температура/заполнение) не могла ничего показать
-- для этой рекомендации. Значения — типовые табличные данные
-- производителя для нейлона с угленаполнением (открытые тех.
-- характеристики eSUN PA-CF): температура сопла 260-280°C, стол
-- 80-100°C (требует подогреваемый стол/камеру), гигроскопичен —
-- требует сухого хранения.
(8, 6, 1, 'eSUN PA-CF', 'Чёрный', 1.75, 0.75, NULL, NULL, 1.12, 260, 280, 90, 90, 1, 1, 0);
