...
CREATE TABLE material (
    ...
    tensile_strength_mpa   NUMERIC(10,2),                  -- предел прочности после указанной термообработки (heat_treatment)
    hardness_hb             NUMERIC(10,2),                  -- твёрдость по Бринеллю (для расчёта режимов резания) — для сталей после закалки на высокую твёрдость (HRC>~40) не заполняется, см. hardness_hrc
    machinability_index     NUMERIC(4,2),                    -- коэфф. обрабатываемости (может переопределять material_group.machinability_index)
    hardness_hrc             NUMERIC(4,1),                    -- твёрдость по Роквеллу (шкала C) — для инструментальных/подшипниковых/быстрорежущих сталей после закалки, где HB не применяется (диапазон измерения)
    red_hardness_c            NUMERIC(6,1),                    -- красностойкость (теплостойкость), °C — сохранение твёрдости при нагреве, актуально для быстрорежущих сталей
    chemical_composition       TEXT,                            -- массовая доля легирующих элементов, % (напр. 'C 0.37-0.44, Cr 0.6-0.9, Ni 1.25-1.65, Mo 0.15-0.25')
    heat_treatment              TEXT,                            -- режим термообработки, к которому относятся приведённые механические свойства
    application                  TEXT,                            -- типовое назначение/область применения марки
    description              TEXT
);
