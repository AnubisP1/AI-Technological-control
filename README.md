...
Возвращает JSON с основной надписью и техническими требованиями чертежа (если передан PDF с текстовым слоем) и геометрией/цветовыми метаданными STEP-модели. Оба файла опциональны — для пластиковых изделий по ТЗ подаётся только `step_model`. Парсинг спецификации и OCR сканов без текстового слоя — не реализованы (см. `PROGRESS.md`).
