...
## Генерация маршрутной карты (Модуль 1.3, упрощённый автоподбор)

```bash
curl -X POST http://localhost:8000/kd/route-card -F "drawing=@чертёж.pdf"
```

Подбирает станки/операции по таблицам совместимости НСИ (материал + тип заготовки → допустимые станки → допустимые операции) и генерирует маршрутную карту по столбцам шаблона ГОСТ 3.1118-82. Упрощённый объём (решение пользователя): без расчёта норм времени/режимов резания и без распознавания формы детали — тип заготовки по умолчанию "прокат сортовой". Операционная карта с переходами не реализована.
