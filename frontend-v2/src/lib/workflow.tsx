 ...
 *
 * stepModel для металла — ОПЦИОНАЛЕН (Фаза 23): чертёж можно отправить
 * на проверку без 3D-модели. Оценка КД, сверка с НСИ, проверки
 * оформления по ГОСТ и маршрутная карта строятся по данным чертежа и не
 * требуют геометрии. Без STEP недоступен только расчёт реального
 * фрезерного тулпаса на "Производстве" (ProductionPage.tsx честно
 * сообщает об этом, а не подменяет расчёт декоративной анимацией).
 */
